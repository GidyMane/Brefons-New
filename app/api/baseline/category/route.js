...
export async function POST(req, res) {
  ...
  const getBaseline = async (countyid, subcategoryid, subindicatorid) => {
    ...
    return baseline[0]?.baseline;
  };

  const getFinal = async (countyid, subcategoryid, subindicatorid) => {
    let { data: baseline, error } = await supabase
      .from("baseline")
      .select("final_target")
      .eq("subcategory_id", subcategoryid)
      .eq("county_id", countyid)
      .eq("subindicator_id", subindicatorid);
    return baseline[0]?.final_target;
  };

  const getMtr = async (countyid, subcategoryid, subindicatorid) => {
    let { data: baseline, error } = await supabase
      .from("baseline")
      .select("mtr_target")
      .eq("subcategory_id", subcategoryid)
    ...
    // let bs = baseline[0]?.mtr_target

    return baseline[0]?.mtr_target;
  };
      ...
      for (const item of subComponent?.data) {
        ...
        if (Array.isArray(item?.Sub_category)) {
          ...
          for (const subCategory of item.Sub_category) {
            ...
            subCategory.baseline = (await getBaseline(
              county_id,
              subCategory.id,
            ...
            subCategory.mtr = (await getMtr(
              county_id,
              subCategory.id,
              item.sub_indicator_id
            ))
              ? await getMtr(county_id, subCategory.id, item.sub_indicator_id)
              : 0;

            subCategory.final_target = (await getFinal(
              county_id,
              subCategory.id,
              item.sub_indicator_id
            ))
              ? await getFinal(county_id, subCategory.id, item.sub_indicator_id)
              : 0;
          }
        }
      }
