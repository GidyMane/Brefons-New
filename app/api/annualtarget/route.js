...
export async function POST(req, res) {
  ...
  const getTarget = async (countyid, subcategoryid, subindicatorid) => {
    ...
    return baseline[0]?.target;
  };

  const getRemarks = async (countyid, subcategoryid, subindicatorid) => {
    let { data: baseline, error } = await supabase
      .from("annual_target")
      .select("remarks")
      .eq("subcategory_id", subcategoryid)
      ...
      for (const item of subComponent?.data) {
        ...
        if (Array.isArray(item?.Sub_category)) {
          ...
          for (const subCategory of item.Sub_category) {
            ...
            subCategory.target = (await getTarget(
              county_id,
              subCategory.id,
            ...
            subCategory.remarks = (await getRemarks(
              county_id,
              subCategory.id,
              item.sub_indicator_id
            ))
