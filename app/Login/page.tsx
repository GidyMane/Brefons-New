    ...
    if (userEmail.trim().length === 0 || userPassword.trim().length === 0) {
      ...
      await axios
        ...
        .then((logins) => {
          ...
          console.log("logins", logins.data);
          if (logins?.data?.status === 400) {
            wronUser();
          } else if (logins?.data?.status === 401) {
            notAuthorized();
          } else if (logins?.data?.status === 200) {
            sessionStorage.setItem(
              "user",
              JSON.stringify(logins?.data?.userdata)
            );
            router.push("/brefons.console");
          }
        })
        .catch((error) => {
          setLoading(false);  // Hide loader
